            ...
            <form onSubmit={submitEditUnit} className="space-y-4">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                <div className="space-y-1">
                  <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wide">Monthly Rent (KES)</label>
                  <div className="flex items-center rounded-lg border border-gray-300 bg-white focus-within:border-indigo-500 focus-within:ring-2 focus-within:ring-indigo-100">
                    <span className="pl-3 pr-1 text-sm font-bold text-gray-500 select-none">KES</span>
                    <input type="number" step="0.01" min="0" value={unitEditForm.rentAmount} onChange={(e) => setUnitEditForm({ ...unitEditForm, rentAmount: e.target.value })} className="flex-1 rounded-lg border-0 py-2 pr-3 text-sm focus:ring-0 bg-transparent" placeholder="e.g. 15000" required />
                  </div>
                  {unitEditForm.rentAmount && Number(unitEditForm.rentAmount) > 0 && (
                    <p className="text-xs text-indigo-600 font-medium">= KES {Number(unitEditForm.rentAmount).toLocaleString()} / month</p>
                  )}
                </div>
                ...
                <form onSubmit={submitUnit} className="space-y-3">
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center rounded-lg border border-gray-300 bg-white focus-within:border-indigo-500 focus-within:ring-2 focus-within:ring-indigo-100">
                        <span className="pl-3 pr-1 text-sm font-bold text-gray-500 select-none">KES</span>
                        <input type="number" step="0.01" min="0" value={unitForm.rentAmount} onChange={(e) => setUnitForm({ ...unitForm, rentAmount: e.target.value })} className="flex-1 rounded-lg border-0 py-2 pr-3 text-sm focus:ring-0 bg-transparent" placeholder="e.g. 15000" required />
                      </div>
                      {unitForm.rentAmount && Number(unitForm.rentAmount) > 0 && (
                        <p className="text-xs text-indigo-600 font-medium">= KES {Number(unitForm.rentAmount).toLocaleString()} / month</p>
                      )}
                    </div>
