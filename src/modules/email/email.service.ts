...
export class EmailService {
  ...
  async sendVerificationEmail(email: string, otp: string, firstName?: string) {
    return this.sendOtpEmail(email, otp, firstName);
  }

  async sendDemoAccessEmail(
    email: string,
    loginEmail: string,
    password: string,
    expiresAt: Date,
...
<body>
  ...
  private getDemoAccessTemplate(
    ...
    const greeting = firstName ? `Hi ${firstName},` : 'Hi,';
    const loginUrl = `${this.configService.get<string>('FRONTEND_URL', 'http://localhost:3001')}/login?mode=demo`;
