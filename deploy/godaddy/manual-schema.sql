...
CREATE TABLE `tenants` (
    ...
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,
    INDEX `tenants_userId_isActive_idx`(`userId`, `isActive`),
    INDEX `tenants_unitId_isActive_idx`(`unitId`, `isActive`),
    UNIQUE INDEX `tenants_userId_unitId_key`(`userId`, `unitId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
-- CreateTable
...
CREATE TABLE `invoices` (
    ...
    `userId` VARCHAR(191) NOT NULL,
    `unitId` VARCHAR(191) NOT NULL,
    `billingType` ENUM('RENT', 'WATER', 'GARBAGE', 'OTHER') NOT NULL,
...
-- AddForeignKey
ALTER TABLE `support_conversations` ADD CONSTRAINT `support_conversations_tenantUserId_fkey` FOREIGN KEY (`tenantUserId`) REFERENCES `users`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
-- AddForeignKey
ALTER TABLE `support_messages` ADD CONSTRAINT `support_messages_conversationId_fkey` FOREIGN KEY (`conversationId`) REFERENCES `support_conversations`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
-- AddForeignKey
