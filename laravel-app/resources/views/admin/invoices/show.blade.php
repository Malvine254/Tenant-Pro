...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px;">
    ...
    <div class="card">
        ...
        <p style="font-size:14px;margin-bottom:4px;"><strong>Base Amount:</strong> KSh {{ number_format($invoice->amount, 2) }}</p>
        <p style="font-size:14px;margin-bottom:4px;"><strong>Penalty:</strong> KSh {{ number_format($invoice->penalty_amount, 2) }}</p>
        <p style="font-size:18px;margin:10px 0 4px;font-weight:700;color:#1d4ed8;"><strong>Total:</strong> KSh {{ number_format($invoice->total_amount, 2) }}</p>
        <p style="font-size:14px;margin-bottom:4px;color:#16a34a;"><strong>Paid:</strong> KSh {{ number_format($invoice->paid_amount, 2) }}</p>
        <p style="font-size:14px;margin-bottom:4px;color:#dc2626;"><strong>Balance:</strong> KSh {{ number_format(max(0, $invoice->total_amount - $invoice->paid_amount), 2) }}</p>
    </div>
</div>
...
<div class="card">
    ...
    <table>
        <thead><tr><th>Date</th><th>Amount</th><th>Method</th><th>Payment Phone</th><th>Status</th><th>Reference / Receipt</th></tr></thead>
        <tbody>
            ...
            <tr>
                ...
                <td><span class="badge badge-gray">{{ $payment->status ?? 'SUCCESSFUL' }}</span></td>
                <td style="font-family:monospace;font-size:12px;">{{ $payment->mpesa_receipt ?? $payment->reference ?? '-' }}</td>
            </tr>
            @empty
            <tr><td colspan="6" style="color:#94a3b8;">No payments yet. Tenant payments will appear here once received.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
