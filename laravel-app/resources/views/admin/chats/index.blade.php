...
<div class="chat-shell" id="chatShell">
 ...
 <a class="conversation {{ $selectedConversation?->id===$c->id?'active':'' }}" href="{{ route('admin.chats.index',['conversation_id'=>$c->id]) }}"><span class="avatar">@if($u?->profile_image_url)<img src="{{ $media($u->profile_image_url) }}" alt="">@else{{ $initials($u?->name) }}@endif</span><span class="conversation-copy"><strong>{{ $u?->name??'Unknown tenant' }}</strong><span>{{ $unit?->property?->name??'No property' }} · Unit {{ $unit?->unit_number??'-' }}</span><span>{{ $last?->body?:$last?->attachment_name?:'No messages' }}</span></span></a>
