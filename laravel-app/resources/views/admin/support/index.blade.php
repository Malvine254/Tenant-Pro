            ...
            <div class="conversation-list" id="conversationList">
                @forelse($conversations as $conversation)
                    ...
                    <a class="conversation-item {{ $selectedConversation?->id === $conversation->id ? 'active' : '' }}" href="{{ route('admin.support.index', array_filter(['conversation_id' => $conversation->id, 'status' => request('status'), 'search' => request('search')])) }}">
                        @php $tenantAvatar = $tenant?->profile_image_url ? $mediaUrl($tenant->profile_image_url) : null; @endphp
                        <span class="conversation-avatar">@if($tenantAvatar)<img src="{{ $tenantAvatar }}" alt="{{ $tenant?->name }}">@else{{ $initials($tenant?->name) }}@endif</span>
            ...
            @if($selectedConversation)
                <header class="chat-header">
                    <div class="chat-person">
                        <span class="conversation-avatar large">@if($selectedTenant?->profile_image_url)<img src="{{ $mediaUrl($selectedTenant->profile_image_url) }}" alt="{{ $selectedTenant->name }}">@else{{ $initials($selectedTenant?->name) }}@endif</span>
                ...
                <form class="reply-box" method="POST" action="{{ route('admin.support.reply', $selectedConversation) }}" id="replyForm">
                    @csrf
                    <input type="hidden" name="client_message_id" id="clientMessageId">
                    <input type="file" name="file" id="replyAttachment" accept="image/jpeg,image/png,image/webp,application/pdf,text/plain,.doc,.docx,audio/*" hidden>
...
<style>
.conversation-avatar img{width:100%;height:100%;border-radius:inherit;object-fit:cover}.attachment-state{font-size:10px;color:#657087}.reply-box.has-attachment textarea{border-color:#7656d8}.attach-button{position:absolute;left:18px;bottom:33px;z-index:2;border:0;background:transparent;color:#6542ca;font-size:20px;cursor:pointer}.reply-box.has-attachment:before{content:attr(data-file-name);position:absolute;left:18px;bottom:4px;max-width:60%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:10px;color:#657087}
</style>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const attachment = document.getElementById('replyAttachment');
    if (replyForm && attachment) {
        const attachButton = document.createElement('button');
        attachButton.type = 'button'; attachButton.className = 'attach-button'; attachButton.textContent = '+'; attachButton.title = 'Attach image, document, or audio';
        replyForm.appendChild(attachButton);
        attachButton.addEventListener('click', () => attachment.click());
        attachment.addEventListener('change', () => {
            const file = attachment.files[0];
            if (file && file.size > 20 * 1024 * 1024) { attachment.value = ''; window.alert('Attachments must be 20 MB or smaller.'); return; }
            replyForm.classList.toggle('has-attachment', !!file);
            replyForm.dataset.fileName = file ? file.name : '';
            reply.required = !file;
        });
    }
    if (replyForm) replyForm.addEventListener('submit', async event => {
        event.preventDefault();
        if ((!reply.value.trim() && !attachment?.files.length) || replyForm.dataset.sending === '1') return;
        document.getElementById('clientMessageId').value = crypto.randomUUID();
        ...
        try {
            ...
            reply.required = true;
            replyForm.classList.remove('has-attachment');
    ...
    // WebSocket broadcasting is not configured in this deployment; use a
    // conservative incremental fallback without ever claiming live presence.
