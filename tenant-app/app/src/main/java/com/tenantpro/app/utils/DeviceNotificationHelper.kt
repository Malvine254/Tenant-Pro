...
object DeviceNotificationHelper {
    ...
    const val FCM_CHANNEL_ID = "tenantpro_default"
    ...
    fun ensureNotificationChannels(context: Context) {
        ...
        // FCM push channel — must match channelId sent by backend.
        // Created here (at app startup) so background FCM messages display correctly
        // without needing onMessageReceived to run first.
        val fcmChannel = NotificationChannel(
            FCM_CHANNEL_ID,
            "Push notifications",
            NotificationManager.IMPORTANCE_HIGH
        ).apply {
            description = "Real-time alerts: payments, invoices, maintenance, and messages"
            enableVibration(true)
            vibrationPattern = longArrayOf(0, 300, 150, 300)
        }
    ...
    fun showPushNotification(
        ...
        showNotification(context, id, title, message, true, destination, entityId)
    }
    ...
    private fun showNotification(
        ...
        message: String,
        priority: Boolean,
        destination: String = "NOTIFICATIONS",
        entityId: String? = null
    ) {
        ...
        val intent = Intent(context, MainActivity::class.java).apply {
            ...
            putExtra(MainActivity.EXTRA_NOTIFICATION_DESTINATION, destination)
            entityId?.let { putExtra(MainActivity.EXTRA_NOTIFICATION_ENTITY_ID, it) }
        }
