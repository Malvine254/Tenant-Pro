...
The app reads the backend server URL from `local.properties` at build time. When your IP address changes, run the helper before rebuilding the app:
...
To update the URL and install the debug app in one step:

```powershell
.\update-backend-ip.ps1 -Install
```

For a physical device explicitly:

```powershell
.\update-backend-ip.ps1 -Target device -Install
```
